...
unsigned int min_gas_towns[MAX] = { 0 } ;
...
int main (int argc, char * argv[])
{
	...
	for (unsigned int i = 0 ; i < roads ; ++i) //for each town find the cheapest road
	{
		...
		if (min_gas_towns[from] == 0 || weight < min_gas_towns[from])
			min_gas_towns[from] = weight;
		if (min_gas_towns[to] == 0 || weight < min_gas_towns[to])
			min_gas_towns[to] = weight;
	}
	fclose(infile);
	...
	{
		if (minimum_gas < min_gas_towns[i]) minimum_gas = min_gas_towns[i];
	}
	printf("%d\n", minimum_gas);
	return 0;
}
