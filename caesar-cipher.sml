...
fun spec_rotn num = (fn x => rotn x num)
fun every_rotn num_list = map spec_rotn num_list
...
fun min l :real = foldl (fn (a,b) => if a<b then a else b) (hd l ) (tl l);
...
val list_rotn = beast_rotn (consecutive (length (explode english_alphabet)));

val text = "Axion esti to heri poy epistrefei apo fono frixton kai tora kserei";

val god_text = ultra_map list_rotn (explode text)

val god_freqs = beast_calc_freqs god_text

val god_entropy = beast_entropy english_frequencies god_freqs

val min_entropy = min god_entropy
